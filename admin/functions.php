        ...
        <section class="calendar__days">
            ...
            for ($cb = 1; $cb <= $boxDisplay; $cb++) {
                if (($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)) {
                    ...
                    global $db;
                    $result = $db->query("SELECT titreActivitees FROM activitees WHERE date = '" . $currentDate . "' AND status = 1");
...
function getEvents($date = '')
{
    ...
    global $db;
    $result = $db->query("SELECT titreActivitees FROM activitees WHERE date = '" . $date . "' AND status = 1");
